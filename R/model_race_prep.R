...
#' Prepare estimation X versus Race Given Name, Location, Outcome, and Covariates
#'
#' Prepares and writes data for use in Python models.
#'
...
#'
#' @return A list containing the prepared data. A sample call will also be printed.
#' @export
model_race_prep = function(r_probs, X, G, Z=NULL, condition=NULL,
    ...
    if (!is.matrix(r_probs)) {
        r_probs = dplyr::select(r_probs, starts_with(prefix))
        names(r_probs) = substring(names(r_probs), nchar(prefix)+1L)
    }
    ...
    out = list(X = data.frame(X=as.integer(X_vec)),
               GZ_mat = as.data.frame(GZ_mat),
               GZ_var = data.frame(GZ_var=as.integer(GZ_var)),
               r_probs = r_probs,
               preds = as.data.frame(preds))

    cat("Call:\n")
    cat(stringr::str_glue("
    fit_additive(<X>, <GZ_mat>, <GZ_var>, <r_probs>, <preds>,
                 {nlevels(X_vec)}, {as.integer(max(GZ_var))}, {{'x': {prior$x}, 'xr': {prior$xr}, 'beta': {prior$beta}}},
                 it={as.integer(config$max_iter)}, epoch={as.integer(config$epoch)}, subsamp={min(length(X_vec), as.integer(config$subsamp))},
                 n_draws={as.integer(config$draws)}, it_avgs={as.integer(config$it_avgs)},
                 n_err={as.integer(min(config$n_err, length(X_vec)))}, lr={config$lr}, tol_rhat={config$tol_rhat},
                 method={method}, silent=False)
                      "))
