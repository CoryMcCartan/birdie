---
title: "Estimation of Racial Disparities when Race is Not Observed: Application to Tax Auditing"
authors: 
    - name: Cory McCartan
    - name: Kosuke Imai
    - name: Jacob Goldin
...
The BISG(Z) procedure assumes that $S\indep G, Z\mid R$. **KI: Let's write this formally as an assumption**
...
\end{theorem}
```
If any racial disparity in $X$ is mediated purely through $G$, $Z$, and $S$, so that $X\indep R\mid G,Z,S$, then the estimator will be unbiased. **KI: Let's also write this formally as an assumption as it will be important to contrast this with our assumption**
...
\end{figure}
```
The DAG implies that $X\indep S\mid R,G,Z$, since all paths from $S$ to $X$ are blocked by $R$, $G$, or $Z$ (for more details, see \citealt{pearl1995causal}). **KI: Let's add another DAG right next to this that represents the usual assumption and then add some discussion.**
Of course, other causal structures, such as a version of Figure \ref{fig:dag} with additional unobserved confounding between $G$, $Z$, $R$, and $X$, are possible; as long as $X\indep S\mid R,G,Z$, then $\Pr(X=x\mid R=r)$ is identified, as we will see below. **KI: let's expand this discussion a bit.  There are three important points. First, this model relaxes the BISG assumption for allowing unobserved confounding between $G$ and $S$ and between $G$ and $Z$.  In addition, unlike the weighting estimator, we can allow for the direct effect of $R$ on $X$.  This is important because you can see this leads to the bias in the weighting estimator.  However, like the weighting estimator, we cannot add unobserved confounding to $X \leftrightarrow Z$ because it will open up the path $R \rightarrow Z \leftarrow U \rightarrow X$ where $Z$ is a collider.  The same argument applies to $G$.**
...
The following theorem records the precise conditions needed for identification. **KI: I think it's worthwhile to walk readers through the basic intuition for this result like you did in your slide while keeping a formal proof in the appendix.  We want people to realize that this identification result is based on a very simple equality.**
...
**KI: Theorem 2.3 is great, but I did not find the discussions right before and after the theorem particularly helpful.  I suggest we comment it out for now unless we can make it more intuitive.**
...
**KI: One general concern I have about this and next sections is that they are based on a very general model/notation.  I worry about losing some readers.  At the same time, I see the value of generality.  Two possibilities.  Start with a simpler model and then present a more general case.  Alternatively, add enough discussions so that people can still build some intuition.**
...
**KI: One interesting observation is that BISGZ makes an additional assumption, which our methodology avoids.  This is $S \indep \{G, Z\} \mid R$. By plugging in BISG probabilities, we are at least partially introducing this assumption.  Unfortunately, this is necessary in order to utilize surname data.  This is why our sensitivity analysis will be useful.  This kind of discussion will be important.**

## Marginalized Posterior

**KI: This section can be combined into the previous section.  In general, we can have one methods section with several subsections.**
...
The bound provided by Theorem \ref{thm:bound} may be computed readily from posterior draws, but it obscures the qualitative features of how the error depends on $\vb*\delta^*$, and in particular the sample size. **KI: Did you check how informative this bound is in the real data?**
