...
abstract: |
    ...
    Leveraging this identification strategy, we introduce a new model, Bayesian Instrumental Regression for Disparity Estimation (BIRDiE), that estimates racial disparity while using surnames as a kind of high-dimensional instrument for race.
    ...
    Our validation study based on North Carolina voter files shows that BIRDiE reduces error by about 74--79% in comparison to existing approaches in estimating party identification by race.
...
We propose using off-the-shelf stochastic variational inference to fit the model to large administrative datasets.
