...
abstract: |
    ...
    Leveraging this identification strategy, we introduce a new class of models, Bayesian Instrumental Regression for Disparity Estimation (BIRDiE), that estimate racial disparity by using surnames as a high-dimensional instrumental variable for race.
    Our estimation method is scalable, making it possible to analyze large-scale administrative data.
    A validation study based on the North Carolina voter file shows that BIRDiE reduces error by up to 84% in comparison to the standard approaches for estimating racial differences in party identification.
...
Where the analysis of racial disparities involves large-scale administrative data, many analysts have adopted Bayesian Improved Surname Geocoding (BISG), which generates individual probabilities of belonging to different racial groups using Bayes’ Rule applied to last names and geolocation [@fiscella2006bisg; @elliott2008bisg; @imai2016improving; @imai2022addressing].
...
As formally discussed in Section \@ref(sec:id), these standard methods of racial disparity estimation based on BISG predictions also require individuals' race to be conditionally independent of the outcome given their residence location, surnames, and other observable attributes.
...
Somewhat counterintuitively, the high-dimensionality of surnames aids rather than hinders identification.
...
We also show how the proposed methodology produces updated BISG probabilities that incorporate the outcome variable, and are more accurate than the BISG probabilities based only on surnames and geolocation.
...
Even though Assumptions \ref{a:indep-sgx} and \ref{a:cens-acc} may not hold exactly, researchers find that BISG produces accurate and generally well-calibrated estimates in practice [@imai2016improving; @kenny2021das; @deluca2022validating], and new methods continue to be developed that increase the calibration of BISG probabilities [@imai2022addressing; @argyle2022misclassification].
...
Unfortunately, accurate and calibrated estimates of individual race predictions alone are not sufficient for unbiased estimation of racial disparities using standard methodologies.
The reason is that the prediction error of race probabilities may be correlated with the outcome variable of interest.
Specifically, consider the following weighting estimator commonly used by researchers, which attempts to capture the uncertainty inherent in race prediction:[^1] $$
...
[^1]: An alternative is a *thresholding* or *classification* estimator, which deterministically assigns individuals to a predicted racial category based on the BISG estimates $\vb{\hat P}_i$ (either the largest $\hat P_{ir}$ or the one which exceeds a predetermined threshold).
    Unlike the weighting estimator, this classification estimator does not take into account prediction uncertainty, and its bias is harder to reason about than that of the weighting estimator \citep{chen2019fairness}.
...
\end{theorem}
```
This result implies that the conditional independence between individual's race and outcome given their surname, residence location, and other characteristics is sufficient to eliminate the asymptotic bias of the weighting estimator.
...
Specifically, we show that racial disparity is identifiable if surname is conditionally independent of the outcome given race, residence geolocation, and other observed information, assuming that BISG can nonparametrically identify race probabilities.
...
We discuss computation for these models, as well as how the methodology can be extended to include an additional explanatory variable that was not used at the BISG stage.
...
For an outcome like party identification, Assumption \ref{a:indep-ys} would mean that among White voters in a particular geographic region, voters named Smith would *a priori* be no more or less likely to identify with one party than voters named Thomas.
...
In this case, Assumption \ref{a:indep-yr} is very likely to be violated, while  Assumption \ref{a:indep-ys} is plausible.
...
The trade-off between the two assumptions is that Assumption \ref{a:indep-ys} rules out the possibility that surname directly affects the outcome whereas such a direct effect is allowed under Assumption \ref{a:indep-yr}.
...
In these cases, both both the weighting estimator and our approach proposed below could give reasonable answers, though the data requirements of each may differ.
...
Such a structure partially pools estimates of $\Pr(Y\mid R, G)$ towards an overall
...
For example, if $G$ records counties, analysts could include predictors for racial and socioeconomic variables measured at the county level.
...
However, the moderate-to-high dimensionality in practical settings makes MCMC algorithms computationally infeasible outside of small datasets.
...
Specifically, given a current parameter estimate $\Theta^{(t)}$, the expected log posterior density can be written as \begin{align*}
    ...
    &= \E_t[\log \pi(\Theta^{(t+1)}, \vb R \mid \vb Y, \vb G, \vb X, \vb S)\mid ] \\
    &= \sum_{\vb r\in\cR^N} 
...
the BISG probabilities updated using Bayes' rule: $$
    \tilde{P}_{ir\mid Y}^{(t)}
...
In combination with the acceleration scheme described next, this allows the entire EM algorithm to be run to convergence on data with hundreds of thousand or millions of individuals in a matter of seconds.
...
This allows for calculation of model-updated BISG probabilities $\tilde{\vb P}_{\mid W}=\pi(\vb R \mid \hat\Theta, \vb W, \vb G, \vb X, \vb S)$, which are also produced as a side effect of the EM algorithm described above.
...
Both these approaches require the following assumption, which generalizes Assumption \ref{a:indep-ys}.
...
In Appendix \ref{app:sens},<!--this section,--> we address this challenge by developing a sensitivity analysis that assesses how the bias in BISG race probabilities affect the estimates of racial disparities.
...
One key result is that a pattern of uniform bias can arise when the prior $\vb q_R$ doesn't match the population distribution of race.
...
In these cases, we recommend that practitioners use other data sources, such as large surveys, to estimate the overall race distribution, and use this instead of national census estimates for $\vb q_R$.

# Empirical Validation and Illustration {#sec:valid}

To better understand how the BIRDiE methodology performs in real-world contexts, we apply it to North Carolina voter registration data.
...
We compare the performance of BIRDiE models against those of other estimators.
We also evaluate how the estimation error depends on the level of geographic precision used in the BISG probabilities.
Finally, we briefly demonstrate various extensions of the BIRDiE methodology: small-area estimates, improved individual race predictions, and estimates conditional on an additional explanatory variable (as discussed in Section \ref{sec:addlcov}).

## North Carolina Voter File

Like most other Southern states which were subject to with a history of disenfranchising minority voters, the state of North Carolina asks (and previously required) every voter to self-report their race upon registration.
...
Estimation of party identification by race is of substantive interest as well, especially in the context of the Voting Rights Act of 1965 (VRA).
...
This sample size is large enough to ensure that sampling error in the estimates is essentially negligible.

Figure \@ref(fig:nc-overview) shows the distribution of party registration by race in this subsample.
White voters disproportionately register as Republicans, while Black voters disproportionately register as Democrats.

## Racial Disparity Estimates 
...
We then compare these results based on these BIRDiE models against those of the two existing estimators --- the weighting estimator as well as a thresholding estimator that deterministically assigns each individual the maximum *a posteriori* racial category.
...
To give an idea of the computational efficiency of the proposed method, the maximum runtime of the saturated of the BIRDiE models fit to estimate party ID was 6.9 seconds (on a modern laptop with 8GB RAM), and the maximum runtime for the mixed model estimates was 22.6 seconds.
We use our software's default priors for both BIRDiE models.
This is a uniform prior for the saturated model and a $\Norm(0, 1)$ prior on $\vb*\beta$ and a $\Gamma(2, 10)$ prior on the random intercept standard deviation for the mixed model.
...
We first examine the relative accuracy of the proposed methods in estimating the disparity between White and Black, and White and Hispanic voters, in party identification.
For example, the true difference in Democratic identification between Black and White voters in the sample is $`r percent(-x$weight$disp_wb_true, 0.1)`$ percentage points, meaning Black voters register Democratic at a much higher rate.
...
For White--Black (left plot) and White-Hispanic (right plot) disparities in party identification, the BIRDiE models (solid circles and squares) substantially outperform the two commonly used estimators (open circles and crosses). 
...
This section examines the accuracy of the saturated and mixed-effects BIRDiE models in recovering small-area relationships between party identification and race, compared to standard methodology that simply applies the weighting and thresholding estimators within each geographic area.
...
We evaluate the small-area estimates by calculating the mean total variation distance between the estimated and true conditional distributions of party ID by race (averaging across geographic areas).
...
Surprisingly, the amount of error does not appear to vary much for the BIRDiE models across different levels of geography---tract-level estimates are roughly as accurate as county-level estimates, on average.
...
As discussed in Section \ref{sec:compute}, we can use the conditional distribution $Y\mid R, X, G$ estimated with a BIRDiE model to create model-updated BISG probabilities $\tilde{\vb P}_{\mid Y}=\pi(\vb R \mid \hat\Theta, \vb Y, \vb G, \vb X, \vb S)$ using Bayes' rule.
...
For example, using the estimates produced by the mixed BIRDiE model applied to party ID with block-level BISG estimates, the MAP prediction accuracy increases from `r acc["block"]` with the input probabilities to `r acc2["mmm"]` with the updated probabilities.
...
One feature of the updated probabilities is that it is appropriate to apply the weighting estimator to them to estimate disparities.
...
In fact, the weighting estimate from to the updated probabilities is identical to the BIRDiE estimate.
While more study is required, for downstream settings where weights are needed, generating these weights with BISG followed by BIRDiE will likely produce more accurate results than simply using BISG weights.
...
We will estimate party identification rates by race among voters and nonvoters in the 2020 election.
Following the discussion in Section \ref{sec:addlcov}, we will compute these estimates two ways: (1) by estimating party ID and 2020 turnout jointly by race, and (2) by first estimating 2020 turnout by race, then estimating party ID by race and 2020 turnout.
...
#|     party identification by turnout and race, by estimation method.
...
In contrast to the BIRDiE models, the weighting and thresholding estimates of party ID by 2020 turnout and race include large errors, especially for Black voters with all the errors exceeding 10 percentage points.
...
Finally, the BIRDiE model could be extended to directly model more complex kinds of outcome variables.
...
It is important to remember that Black and White voters together comprise over 95% of the sample.
In general, BIRDiE is more accurate than the other methods for the White and Black racial groups which together make up 96% of the sample.
All estimators perform roughly equally well for Hispanic, Asian, and Native voters (though the thresholding estimator performs well for Asian voters in estimating party ID).
The complete-pooling model performs poorly for the "Other" racial group, though no method performs particularly well for that group.
...
# Appendix: Additional Small-area Accuracy Evaluation {#app:small}
...
# Appendix: Sensitivity Analysis {#app:sens}
