...
abstract: |
    ...
    Although BISG tends to produce well-calibrated racial predictions, its residuals are often correlated with the outcomes of interest, yielding biased estimates of racial disparities, often in the direction of minimizing the disparity.
...
As formally discussed in Section \@ref(sec:id), these standard methods of racial disparity estimation based on BISG predictions implicitly require individuals' race to be conditionally independent of the outcome given their residence location, surnames, and other observable attributes.
...
In fact, the weighting estimator will often underestimate the magnitude of a disparity, as the following corollary makes precise. Thus for instance in measuring disparities in loan approval ($Y$), if Black respondents are less likely to be approved for loans across all locations and surnames, then the weighting estimator would understate the resulting overall White-Black disparity in loan approval rates.
...
The same kind of assumption is made in the analysis of surrogate outcomes in causal inference, where the effect of treatment on the outcome is assumed to occur entirely through a set of surrogate variables [see, e.g., @athey2016estimating].
