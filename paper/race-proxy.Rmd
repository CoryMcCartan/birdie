...
abstract: |
    ...
    Our validation study based on North Carolina voter files shows that BIRDiE reduces error by about 75--85% in comparison to existing approaches in estimating party identification by race.
...
BIRDiE yields an estimate of `r round(-100*x$model$disp_wb)` percentage points.
...
The OLS estimator (yellow triangles) outperforms the weighting and threshold estimators for party ID, but performs significantly worse for turnout than the weighting estimator. **CM CHECK NEW**
