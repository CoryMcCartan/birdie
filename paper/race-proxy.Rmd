...
Note that individual's race is unobservable but all other variables are observed although the availability of $X$ is not required for the proposed methodology.
...
The BISG estimator of the probability that individual $i$ belongs to race $r \in \cR$ can then be written as [@fiscella2006bisg; @elliott2008bisg] 
\begin{equation} \label{eq:pr-bisg}
    \hat{P}_{ir} \dfeq \frac{q_{G_iX_i|r}\, q_{S_i|r}\, q_r}{
        \sum_{j\in\cR}q_{G_iX_i|j}\, q_{S_i|j}\, q_j},
\end{equation}
where, for example, $q_{G_iX_i|r}$ indicates the estimated conditional probability of residence location $G_i$ and covariates $X_i$ given race $r$, taken from the Census table $\vb q_{GX|R}$.
...
These groups, however, have distinct sets of surnames and may have different demographic and geographic distributions.
Unlike the Smith example, therefore, knowing that an Asian individual's surname is Gupta makes it much more likely that they have a higher income and live in the Eastern U.S [@budiman2019key].
...
Even though Assumptions \ref{a:indep-sgx} and \ref{a:cens-acc} are never perfectly met, researchers find that BISG produces accurate and generally well-calibrated estimates in practice [@imai2016; @kenny2021das; @deluca2022validating].
...
Specifically, we consider the following weighted estimator, which attempts to capture the uncertainty inherent in race prediction,^[An alternative is a classification estimator, which deterministically assigns individuals to a predicted racial category based on the BISG estimates $\vb{\hat P}_i$ (either the largest $\hat P_{ir}$ or the one which exceeds a predetermined threshold). Unlike the weighted estimator, this classification estimator does not take into account uncertainty.]
$$
...
Formally, \citet{chen2019fairness} shows that the asymptotic bias of this weighted estimator is controlled by the residual correlation of $Y$ and $R$ after controlling for $G$, $X$, and $S$.
...
So long as we observe all information used by the manager, the weighted estimator would give an asymptotically unbiased estimate.
...
Figure \@ref(fig:dag)(b) shows one possible causal structure that meets this assumption as well as Assumption \ref{a:indep-sgx}.**KI: We should remove dotted arrows from the figure. I realized that they violate the BISG assumption.**
In this DAG, race can have a direct effect on the outcome $Y$ as well as on residence location $G$ and other observed chracteristics $X$, while all paths from $S$ to $Y$ are blocked by $G$, $X$, or $R$.
...
Assumption \ref{a:indep-ys} also implies that there is no unobserved confounder affecting both surname and outcome. **KI: What would be an example of this?**
...
\end{equation*}
where the parameters $\theta_{yr}(g,x)$ represent the probabilities for each level of the outcome $Y$, which can vary by $R$, $G$, and $X$.
...
Specifically, we model $S$ and $(G,X)$ separately given $R$ using the Census data under Assumptions \ref{a:indep-sgx} and \ref{a:cens-acc}): 
\begin{align*}
...
\end{align*}
Finally, the model is completed with the prior distribution for $\Theta$, which is denoted by $\pi(\Theta)$ and discussed in detail below.

Putting all together, the posterior distribution can then be written as 
\begin{align}
...
\end{align} 
where as above $\hat{\vb P_i}$ are the BISG probability estimates for individual $i$, which depend on Census data represented in $\vb q_{GX\mid R}$, $\vb q_{S\mid R}$, and $\vb q_R$, but not on the parameters $\Theta$ and individual race $\vb R$.
...
\end{align*}
This prior, however, suffers from the curse of dimensionality as more covariates are added to $X$, especially since in practice $G$ will be quite large, covering many blocks or ZIP codes.
...
**KI: Two things that need to be done here.  First, it is unclear how marginalization incorporates the prior, which has $R$.  Second, we should include some experiments you mention here (the results could be given in the appendix).** 

Since $\vb R$ is high-dimensional, discrete, and not of primary interest, we can marginalize it out as follows: 
\begin{align*}
...
\end{align*} 
where the second equality is due to Theorem 2.1 of @giordano2018cov [see also the idea of *local sensitivity* from @gustafson1996local].
...
This bound is proportional to the covariance between the conditional probability of the outcome $Y$ given race $R$ and residence location $G$ and its normalized version. **KI: any intuition here??**
...
**KI: I'm thinking of getting rid of the following result. It seems to me that the above result subsumes the latter.  What do you think?**
...
We use a subset of the June 2022 voter file which could be linked to a proprietary voter file provided by L2, Inc. a leading national non-partisan firm and the oldest organization in the United States that supplies voter data and related technology to candidates, political parties, pollsters, and consultants for use in campaigns.
...
$$The TV distance is an upper bound on the error in *any* probability calculated from the estimated joint distribution, and as such is useful general-purpose measure of estimation error. **KI: Can we also compute other measures? What about putting false negative and false positive rates into Appendix.**
...
**KI: We should start with the estimates of racial disparities themselves. I think that partisanship and turnout differentials would be good things to show. Then, measure errors via TV distance. **
...
The OLS estimator outperforms the weighted estimator for party ID, but the situation is reversed for turnout. **KI: How are you modeling the turnout?**
...
For the county-level party ID estimates, a total error of $\Delta=`r number(x[["tot_err_party"]], 0.001)`$ is sufficient to explain the largest error in the joint distribution. **KI: Not sure exactly what this means. Can we show via a graph how changing $\Delta$ leads to different estimates of disparities? I'm also worried about if this sensitivity analysis may not be too informative...**
