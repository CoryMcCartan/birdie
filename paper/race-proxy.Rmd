...
This bound is proportional to the covariance between the conditional probability of the outcome $Y$ given race $R$ and residence location $G$ and its normalized version.
**KI: any intuition here??**
...
**KI: I'm thinking of getting rid of the following result. It seems to me that the above result subsumes the latter. What do you think?** For a more qualitative understanding of the effect of a particular $\vb\delta$, we can derive results on the direction of the error in $\E_\pi[g(\Theta)]$ for linear $g(\Theta)=\vec(\Theta)^\top\vb d$ and particular configurations of $\vb\delta$.
...
**KI: Can we also compute other measures? What about putting false negative and false positive rates into Appendix.** Figures \@ref(fig:nc-fit-party) and \@ref(fig:nc-fit-turnout) show the TV distance for each estimator across the range of geographic levels used in the BISG calculation.
...
**KI: How are you modeling the turnout?** This suggests that compared to the effect of race on party ID, the effect of race on turnout is more strongly moderated by geography, reducing the size of the violation of Assumption \ref{a:indep-yr}.
...
**KI: Not sure exactly what this means. Can we show via a graph how changing** $\Delta$ leads to different estimates of disparities?
I'm also worried about if this sensitivity analysis may not be too informative... For the county-level turnout estimates, a total error of $\Delta=`r number(x[["tot_err_turnout"]], 0.001)`$ suffices.
