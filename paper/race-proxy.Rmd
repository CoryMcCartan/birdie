...
abstract: |
    The estimation of racial disparities in health care, financial services, voting, and other contexts is often hampered by the lack of racial information in administrative records. As a result, many analysts have adopted Bayesian Improved Surname Geocoding (BISG), which uses individual names and addresses to predict race. Although BISG tend to produce well calibrated racial predictions, its prediction errors are often correlated with the outcomes of interest, yielding biased estimates of racial disparities. We propose a new identification strategy that corrects this bias in estimating racial disparity via BISG's race predictions.  The proposed strategy is applicable whenever one's surname is conditionally independent of the outcome given their (unobserved) race, residence location, and other observed characteristics. Leveraging this identification strategy, we introduce a new Bayesian model that estimates racial disparity while accounting for the high-dimensionality of surnames.  Lastly, we also develop a sensitivity analysis to address the potential violation of the required assumption that BISG's race predictions are well calibrated.  Our validation study based on voter files shows that the proposed model achieves 76–85% lower error than existing approaches in estimating party identification by race in North Carolina, and 24–49% lower error for turnout rates by race. A similarly substantial improvement is obtained in estimating racial disparities of partisan identification.  We also apply the model to large-scale IRS administrative data to produce the first-ever estimates of claim rates of the home mortgage interest deduction by race.  We find that ... Open-source software is available for implementing the proposed methodology.
keywords: 
    - BISG
...
**from Dan: "Do we need a more informative title? Or is our point that we're clarifying assumptions implicit in a range of existing approaches?" (CM: I think the latter)**

# Introduction

The identification and estimation of racial disparities is of critical importance to researchers, policymakers and organizations in areas including public health [@van2003paved; @williams2005social], employment [@conway1983reverse; @greene1984reverse], voting [@gay2001effect; @hajnal2005turnout; @barreto2007isi], criminal justice [@berk2021fairness; @chouldechova2017fair; @dressel2018accuracy], taxation [@black2022algorithmic], housing [@kermani2021racial], and lending [@chen2018fair].
Within the U.S. government, efforts to identify and remedy racial disparities have taken on greater urgency with the recent issuance of Executive Order 13985, which in part directs agencies to develop methods to assess equity and conduct equity assessments.

But for data in many of these areas, racial information is not present at the individual level.
This prevents analysts from simply tabulating variables of interest against race to identify disparities.
In fact, in some areas, the law prohibits the collection of racial information even as it demands fair treatment on the basis of race (see, e.g. the U.S. Equal Credit Opportunity Act).
This creates a dilemma for organizations who wish to measure possible disparities in order to monitor the fairness of their decisionmaking or service provision.

To estimate racial disparities without individual-level racial data, may researchers turn to ecological inference methods [@goodman1953ecological; @king1997solution; @king2004ecological; @wakefield2004ecological; @imai2008bayesian; @greiner2009r].
However, these methods require sometimes-strong assumptions which can be difficult to verify.
...
BISG leverages residential racial segregation and the hereditary association between race and surname to produce generally accurate and calibrated predictions of individual race [@imai2016improving; @deluca2022validating].

While BISG produces accurate racial predictions, these are not sufficient for estimating disparities accurately.
The most common techniques for estimating disparities using BISG probabilities are known to be biased [@chen2019fairness].
These biased estimators are widely used, with potentially large real-world consequences.

Bias arises in the use of BISG probabilities because of a disconnect between the relationship between the variables in real-world data and the implicit assumptions that underlie common BISG-based disparity estimators.
In Section \@ref(sec:id), we clarify these implicit assumptions and relate them to the causal structure underlying the data.
We discuss how the assumptions needed for the most common BISG disparity estimator to be unbiased are unlikely to be met in practice.
Instead, we offer an alternative assumption and a new identification condition for the joint distribution of an outcome variable and race when the latter is unobserved.

In Section \@ref(sec:est), we develop a Bayesian model that uses this new identification condition to accurately estimating racial disparities.
We can fit the model on large administrative datasets using stochastic variational inference.
This is implemented in open-source software that is made available with the paper.
...
Section \@ref(sec:sens) proposes a method to evaluate the sensitivity of inferences about racial disparities to model assumptions.

Section \@ref(sec:valid) validates the proposed methods on real-world data taken from the voter file in North Carolina, where individual-race is observed and can be used as a ground-truth baseline.
...
We produce the first-ever estimates of claim rates of the home mortgage interest deduction (HMID) by race.... [fill in].
Section \@ref(sec:disc) concludes.

# Identification {#sec:id}

In this section, we discuss the proposed identification strategy.
...
For each individual $i=1,2,\ldots,N$, we define a tuple $(Y_i, R_i, G_i, X_i, S_i)$, where $Y_i\in\cY$ is the outcome of interest for individual $i$, $R_i\in \cR$ is the (unknown) race of the individual, $G_i\in\cG$ is the (geo)location of the individual's residence, $X_i\in\cX$ are other observed characteristics, and $S_i\in\cS$ is the individual's surname.
When we are not referring to a particular individual, we will drop the subscripts.
Note that individual's race is unobservable but all other variables are observed, although the availability of particular (or any) $X$ is not required for the proposed methodology.
...
For the sake of simplicity, we assume that the outcome variable $Y$ is also discrete, though it is possible to extend the proposed methodology to continuous outcome variables.
...
\end{assump}
```
Assumption \ref{a:indep-sgx} implies, for example, that once we know an individual is White, knowing their surname is Smith tell us nothing about their residence location and other characteristics.
...
These groups, however, have distinct sets of surnames and have different demographic and geographic distributions.
...
The second assumption of BISG is that the Census tables reflect the true population distributions of $R$, $S$, $G$, and $X$.
...
$$ justifying the estimator given in \eqref{eq:pr-bisg}, and providing us with the following immediate result.

```{=tex}
\begin{prop}[Accuracy of BISG] \label{p:bisg}
Under Assumptions \ref{a:indep-sgx} and \ref{a:cens-acc}, the BISG estimator is accurate:
$\hat P_{ir} = \Pr(R_i=r\mid G_i,X_i,S_i)$.
\end{prop}
```
## Bias of Racial Disparity Estimates based on BISG Probabilities

Unfortunately, accurate and calibrated estimates of $\Pr(R\mid G,X,S)$ are not sufficient for identifying racial disparities.
The reason is that the prediction error of race probabilities may be correlated with the outcome variable of interest.
Specifically, we consider the following weighted estimator, which attempts to capture the uncertainty inherent in race prediction,[^1] $$
...
[^1]: An alternative is a classification estimator, which deterministically assigns individuals to a predicted racial category based on the BISG estimates $\vb{\hat P}_i$ (either the largest $\hat P_{ir}$ or the one which exceeds a predetermined threshold).
    Unlike the weighted estimator, this classification estimator does not take into account uncertainty.

\citet{chen2019fairness} show that the asymptotic bias of this weighted estimator is controlled by the residual correlation of $Y$ and $R$ after controlling for $G$, $X$, and $S$.
...
\end{theorem}
```
This result implies that the conditional independence between individual's race and outcome given their surname, residence location, and other characteristics is sufficient to eliminate the bias of the weighted estimator.

```{=tex}
\begin{assump}[Conditional independence of outcome and race] \label{a:indep-yr}
For all individuals $i$, $Y_i\indep R_i\mid G_i,X_i,S_i$ holds.
\end{assump}
```
Figure \@ref(fig:dag)(a) shows a causal structure that satisfies Assumption \ref{a:indep-yr} as well as Assumption \ref{a:indep-sgx}.
The DAG implies that $Y\indep R\mid G,X,S$ because all paths from $R$ to $Y$ are blocked by $G$, $X$, or $S$.
...
This type of exclusion restriction is difficult to satisfy in many practical settings because race may affect so many factors, biasing the weighted estimator.
...
So long as we observe all information used by the manager and use it in the BISG estimation, the weighted estimator would give an asymptotically unbiased estimate.
Outside of these cases, however, the weighted estimator is likely to be biased.
...
To reduce this potential bias of the weighted estimator, we propose an alternative identification assumption that may be applicable when Assumption \ref{a:indep-yr} is not credible.
...
\end{assump}
```
Figure \@ref(fig:dag)(b) shows one possible causal structure that meets this assumption as well as Assumption \ref{a:indep-sgx}.
...
This structure is plausible in many real-world settings, such as our motivating examples of HMID claims and voter turnout.
In both these cases, we can rule out a direct effect of name on the outcome.
(**CM: revisit this more carefully once we've settled on outcomes. Dan notes e.g. mortgage application success may depend on discrimination via names**.) This assumption rules out the possibility that surname directly affects the outcome whereas such a direct effect is allowed under Assumption \ref{a:indep-yr}.
...
Assumption \ref{a:indep-ys} also implies that there is no unobserved confounder affecting both surname and outcome.
**KI: What would be an example of this? CM: I think most violations of** $Y\indep S\mid G, R$ **will take this form---something like income or the region in a country you immigrated from could affect your surname and the outcome.**

In any particular application, therefore, it is important to consider whether it is more reasonable to rule out the direct effect of race on the outcome or the direct effect of surname on the outcome.
In the former, Assumption \ref{a:indep-yr} is more appropriate whereas in the latter, Assumption \ref{a:indep-ys} is more credible.
...
\end{proof}
```
The essence of this identification result is the following observation: under Assumption \ref{a:indep-ys}, we have, for all $y\in\cY$, $g\in\cG$, $x\in\cX$, and $s\in\cS$, \begin{equation} \label{eq:tprob}
    \Pr(Y=y\mid G=g, X=x, S=s)
    = \sum_{r\in\cR} \Pr(Y=y\mid R=r, G=g, X=x)\Pr(R=r\mid G=g, X=x, S=s).
...
Our result is closely related to the causal identification based on proxy variables in the presence of unmeasured confounding [@kuroki2014measurement; @miao2018identifying].
Here, we use surname as a proxy variable for race.
...
\end{proof}
```
It is worth comparing this OLS estimator with the weighted estimator $\hat{\mu}^{(\text{wtd})}_{y\mid r}$.
The next theorem shows that these two estimators are guaranteed to disagree, unless either the BISG probabilities perfectly discriminate **(KI: Doesn't this simply mean that one racial category has a probability of one and the other categories have a probability of zero?** **CM: in the two-race case, yes; but you could split e.g. 50%/50%/0% and satisfy the preconditions for some of the pairs)** or the weighted estimator is constant across races.
These two conditions are almost never met in practice.
...
\end{proof}
```
Despite potential advantages over the weighted estimator, the OLS estimator is not well-suited to estimation in practice, since it ignores the fact that the unknown parameters are probabilities and thus constrained to be nonnegative and sum to 1.
...
In this section, we propose a Bayesian model that can be used to estimate racial disparities under the proposed alternative identification strategy.
...
**KI: Two things that need to be done here. First, it is unclear how marginalization incorporates the prior, which has $R$.**
**CM: The prior is subsumed into $\hat{P}_{ir}$, which is maintained through the equations below. I think originally I had broken out the marginalization in to more steps, which might help explain it better.**

Since $\vb R$ is high-dimensional, discrete, and not of primary interest, we can marginalize it out as follows: \begin{align*}
...
The proposed model, like the weighted and OLS estimators, relies upon Assumptions \ref{a:indep-sgx} and \ref{a:cens-acc}, which are required for the BISG race probabilities to be accurate.
Unfortunately, these assumptions do not exactly hold in practice.
...
We begin by pointing out that both Assumptions \ref{a:indep-sgx} and \ref{a:cens-acc} enter our model solely through the BISG race probabilities $\hat{\vb P}$.
If the Census data are inaccurate, or if the conditional independence relation does not hold $S\notindep G,X\mid R$, then the calculated probabilities $\hat{\vb P}$ will differ from the "true" individual race probabilities $\vb P^*$.
...
Thus the overall impact of the sample size on the bound in Equation \eqref{eq:covbound} is unclear and may depend on specific features of the data.
...
**KI: I'm thinking of getting rid of the following result. It seems to me that the above result subsumes the latter. What do you think? CM: I added clarification that these results are *not* asymptotic but rather exact inequalities. I think that makes them helpful.** 
...
We compare the performance of the proposed model against those of other estimators, and evaluate how the estimation error depends on the level of geographic precision used in the BISG probabilities.
...
We use a subset of the June 2022 voter file which could be linked to a proprietary voter file provided by L2, Inc. a leading national non-partisan voter data and technology firm.
The L2 file geocoded each address to a Census block, which is necessary for the finest block-level BISG predictions.
...
The overall merged voter file contains 5,711,795 voters, 71.2% of which are White, 21.1% of which are black, and 7.7% of which belong to another race.
...
For each set of BISG probabilities, we estimate the joint distribution of each outcome variable and race using the proposed method, the weighted estimator, the OLS estimator, and a thresholding estimator that deterministically assigns each individual the maximum *a posteriori* racial category.
...
For example, the true difference in Democratic identification between White and Black voters in the sample is -`r percent(-x$model$disp_wb_true)`, meaning Black voters register Democratic at a much higher rate.
However, the standard weighting approach produces an estimate of only -`r percent(-x$weight$disp_wb)` for this disparity---less than half the true value.
In contrast, the proposed model produces an estimate of -`r percent(-x$model$disp_wb)`.
Figure \@ref(fig:nc-disp) compares all the methods across all of these possible disparity measurements, using the Census block-level BISG predictions.
The true disparity is subtracted off for ease of comparison.
Across both party and turnout, and both inter-racial comparisons, the proposed method outperforms the other three estimators.
...
**KI: Can we also compute other measures? What about putting false negative and false positive rates into Appendix.**
**CM: What do you mean by FNR and FPR in this context?**
Figure \@ref(fig:nc-tv) shows the TV distance for each estimator, not just for the Census block-level BISG estimates used in Figure \@ref(fig:nc-disp) but also across the range of geographic levels used in the BISG calculation.
...
The OLS estimator outperforms the weighted estimator for party ID, but the situation is reversed for turnout.
...
However, the weighted estimator is probably the "least bad" for this group.
