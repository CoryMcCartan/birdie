...
Unfortunately, calibrated BISG racial prediction alone is not sufficient for unbiased estimation of disparities.
...
Conversely, conditional independence between individual's race and outcome given their surname, residence location, and other characteristics is sufficient to eliminate the asymptotic bias of the weighting estimator.
...
Just as with BISG, well-calibrated probabilities are not generally sufficient to produce unbiased estimates of racial disparities using the outputs of these methods.
...
The trade-off between the two assumptions is that Assumption \ref{a:indep-ys} rules out the possibility that surname directly affects the outcome; such a direct effect is allowed under Assumption \ref{a:indep-yr}.
...
Both these approaches require the following identifying assumption, which generalizes Assumption \ref{a:indep-ys}.
...
This section, along with Appendix \ref{app:sens}, develop a sensitivity analysis that assesses how violations of these assumptions affect the estimates of racial disparities.

##### Violations of Assumption \ref{a:indep-ys}.

BIRDiE assumes that conditional on unobserved race and observed covariates, outcomes and surnames are independent.
We can partially relax this assumption for the purposes of sensitivity analysis.

Consider a low-dimensional summary statistic of surname, $f:\cS\to\R^d$, $d\ll|\cS|$.
Then even if Assumption \ref{a:indep-ys} does not hold, the following weakened assumption might hold.
...
It is still possible to nonparametrically identify racial disparities under Assumption \ref{a:indep-ysf}, though the identification condition is slightly stronger than for Theorem \ref{thm:id}.
...
Assumption \ref{a:indep-ysf} and Theorem \ref{thm:id2} can be used in conjunction with carefully chosen $f$ in order to probe likely failure modes of the more restrictive Assumption \ref{a:indep-ys}.
As discussed in Section \ref{subsec:ident}, association between the outcome and country of origin or racial subgroups may lead to correlation between surnames and outcome even after controlling for race and geography.
...
##### Violations of Assumptions \ref{a:indep-sgx} and \ref{a:cens-acc}.

Bias can also arise from violations of the assumptions underlying the BISG methodology.
...
#| fig.scap: Distribution of party identification by race for North Carolina voters
...
#| fig.scap: Error in racial disparity estimates for party identification, by estimation method
x = readRDS(here("paper/data/nc_disp_ex.rds"))
...
#| fig.scap: |
#|     Total variation distance between the estimated and actual distribution of
#|     party identification, by estimation method and level of geographic detail 
#|     used in the BISG predictions
...
#| fig.scap: Accuracy of small-area party identification estimates by race
...
Finally, we can examine the sensitivity of our party identification estimates to violations of the key identifying Assumption \ref{a:indep-ys}, following the method outlined in Section \ref{sec:sens} of using a low-dimensional surname summary statistic.
Using a publicly available sample of 5\% of the individual records for the 1930 Census [@cens1930], which contains individual names, individual and parental birthplace, and detailed race, ethnicity, and tribal codes, we can classify most surnames in the voter file into nine groups.
We further supplement this data with around 3,000 Asian surnames classified into six regional subgroups [@asiannames], since many regions of Asia, particularly Vietnam, experienced little emigration to the United States before 1930.
While necessarily somewhat arbitrary, these groups are chosen to combine countries of origin which had significant immigration to the United States during similar periods.
A high-level description of the groupings, and the most common 50 surnames for each group, may be found in Appendix \ref{app:surgrp}.

We can first evaluate the plausibility of Assumption \ref{a:indep-ys} by examining the correlation between the residuals of the BIRDiE model fit and indicator variables for each of the nine surname groups.
Under Assumption \ref{a:indep-ys}, this residual correlation should be zero everywhere.
As Figure \ref{fig:nc-sens} shows, however, for many groups and party labels, the correlation is nonzero and more extreme than would be expected given only sampling variation.
...
```{r nc-sens}
#| fig.cap: |
#|     Residual correlation between party identification and nine surname groups,
...
Notably, voters with names in the `ANGLOBL` group, which includes surnames that are relatively more common among many-generation residents of the U.S., such as Smith, Williams, and Brown, are significantly less likely to register as Democrats and independents, and more likely to identify as Republicans, even after controlling for race and geography.
Meanwhile, voters with names in the `EUR_WAVE1` and `EUR_WAVE2` groups, which includes surnames more common among 19th and 20th century immigrants from Europe, display the opposite pattern.
...
Thus we might expect our party-by-race estimates to be little affected by the inclusion of the surname group indicators.
...
This re-fitting requires assuming that Assumption \ref{a:indep-ysf} holds rather than Assumption \ref{a:indep-ys}.
Violations of Assumption \ref{a:indep-ys} that do not satisfy Assumption \ref{a:indep-ysf} are therefore not able to be accounted for.
Nevertheless, the average party identification rate estimate changes by `r number(mean(abs(x)), 0.001)`, with the largest change being 
`r number(max_x, 0.001)`, for the rate of `r max_party` identification among `r max_race` voters.
All in all, this analysis provides confidence that violations of Assumption \ref{a:indep-ys} for the North Carolina voter file are likely minor and would have minimal effects on quantiative and qualitative conclusions.
...
But we expect many of the qualitative conclusions to hold for BIRDiE models.
...
# Surname Groupings for North Carolina Sensitivity Analysis {#app:surgrp}
...
We collapse the 22 surname groups to nine for the sensitivity analysis in Section \ref{sec:valid} based on the demographics of North Carolina specifically and to minimize the computational burden of performing the sensitivity analysis.
