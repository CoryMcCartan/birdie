...
Despite potential advantages over the weighted estimator, $\hat{\vb*\mu}^{(\text{ols})}_{Y\mid R}$ is not well-suited to estimation in practice since it ignores the fact that the unknown parameters are probabilities and thus constrained to be nonnegative and sum to 1.
...
To address this challenge, and to open the door to more flexible modeling, we propose a simple Bayesian model below.
