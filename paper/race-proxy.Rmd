...
If race is binary (so $\cR=\{0,1\}$), then as $N\to\infty$, \[
    \hat\mu^{(\text{wt})}_{Y|R}(y\mid r) - \Pr(Y=y\mid R=r)
...
\begin{align*}
    \E[\ind\{Y=y\}\mid \vb{\hat p}] & = \E[\E[\ind\{Y=y\}\mid G, X] \mid \vb{\hat p}] \\
...
\end{proof}
```
It is worth comparing this OLS estimator with the weighting estimator $\hat{\mu}^{(\text{wt})}_{y\mid r}$.
...
For any $y\in\cY$, $\hat{\vb*\mu}^{(\text{wt})}_{Y|R}(y\mid\cdot) = \hat{\vb*\mu}^{(\text{ols})}_{Y|R}(y\mid\cdot)$
...
$\hat{\mu}^{(\text{wt})}_{Y|R}(y\mid j)=\hat{\mu}^{(\text{wt})}_{Y|R}(y\mid k)$.
\end{theoremrep}
\begin{proof}
Fix a $y\in\cY$.
...
For the forward direction, we assume $\hat{\mu}^{(\text{wt})}_{Y|R}(y\mid j)=\hat{\mu}^{(\text{ols})}_{Y|R}(y\mid j)=c_j$;
