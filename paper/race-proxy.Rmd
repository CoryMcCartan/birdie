...
abstract: |
    ...
    We also show how to address the potential violations of the key identification assumptions, including name-based discrimination.
...
BIRDiE also produces updated BISG probabilities that incorporate the outcome variable, and are more accurate than the BISG probabilities based only on surnames and geolocation.
Finally, we show how to address the potential violations of the key identification assumptions, including name-based discrimination, by exploiting auxiliary information about the relations between names and finer ethnic groups.
...
New methods continue to be developed that improve the calibration of BISG probabilities, including some machine learning methods based on labeled data [@imai2022addressing; @argyle2022misclassification; @decter2022].
...
Dashed node signifies the fact that race $R$ is unobserved.
...
Possible causal structures for which each of the labeled assumptions is satisfied, represented as a directed acyclic graph (DAG) where $G$ is residence location, $R$ is race, $S$ is surname, $X$ is observed covariates, and $Y$ is the outcome. Race $R$ is unobserved, which is signified by dashed node. Both DAGs also satisfy Assumption \ref{a:indep-sgx}: Conditional independence of $S$ and $(G,X)$ given $R$.}
\label{fig:dag}
\end{figure}
```
...
Finally, we show how to addresses the potential violations of the key identifidation assumptions, including name-based discrimination.
...
For example, if $G$ records counties, analysts could include as predictors racial and socioeconomic variables measured at the county level.
...
However, the moderate-to-high dimensionality in practical settings, even after integrating out $\vb R$, makes MCMC algorithms computationally too expensive outside of small datasets.
...
Another scenario is a policy evaluation study where researchers are interested in how the impact of policy varies across racial groups, requiring the incorporation of interaction between race and the treatment variable.
...
However, the standard weighting approach produces an estimate of only $`r percent(-x$weight$disp_wb, 0.1)`$ percentage points for this disparity---less than half the true value. **KI: Let's also describe the estimates based on the thresholding estimator.**
...
**KI: We should be consistent across figures.  How about using percentage points for Figures 4 and 5 as well? Also, for Figure 5, let's extend the y-axis to 0 like the other figures.**
