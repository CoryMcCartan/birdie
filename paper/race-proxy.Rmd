...
Lastly, the remaining term in the middle can be solved for, since Equation \eqref{eq:tprob} holds across all combinations of $Y$, $G$, $X$, and $S$, leading to a large system of equations. **KI: Therefore, we have a total of $(C_Y - 1) \times C_G \times C_X \times C_S$ equations with $(C_y - 1)\times C_R \times C_G \times C_X$ unknown parameters.  Since we typically have $C_R << C_S$, we can identify these unknowns.**
...
We first present the BIRDiE model and describe the choice of prior before briefly discussing computational issues.
...
For example, $\vb*\theta_{\cdot R_i}(G_i,X_i)$ is a vector of length $C_Y$ consisting of entries of $\theta_{yR_i}(G_i,X_i)$ for $y\in\cY$.
...
The final but critical component of the BIRDiE model is the prior $\pi(\Theta)$.
...
Under this assumption of no higher-order interaction, we propose the following *additive* prior specification, which can be viewed as a random intercept model: \begin{align*}
...
All together, `r percent(1 - 5754912/7389159, 0.1)` of records either had missing race information or could not be linked to the L2 file.
