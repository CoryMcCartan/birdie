...
abstract: |
    ...
    Although BISG tends to produce well-calibrated racial predictions, its residuals are often correlated with the outcomes of interest, yielding biased estimates of racial disparities.
    ...
    Leveraging this identification strategy, we introduce a new class of models, Bayesian Instrumental Regression for Disparity Estimation (BIRDiE), that estimate racial disparity while using surnames as a high-dimensional instrument for race.
    We also develop a sensitivity analysis to address potential violations of the required assumption that BISG's race predictions are well calibrated.
    Our validation study based on North Carolina voter files shows that BIRDiE reduces error by up to 94% in comparison to the standard approaches for estimating racial differences in party identification.
keywords: 
    - BISG
...
bias_birdie = with(x$model, disp_wb - disp_wb_true)
...
This creates a dilemma for organizations who wish to measure possible disparities in order to monitor the racial disparity of their decision-making or service provision.
...
Where the analysis of racial disparities involves large-scale administrative data, many analysts have adopted Bayesian Improved Surname Geocoding (BISG), which generates individual probabilities of belonging to different racial groups using Bayes’ Rule applied to last names and geolocation [@fiscella2006bisg; @elliott2008bisg; @imai2022addressing].
...
Unfortunately, the accurate BISG racial prediction alone is not sufficient for unbiased estimation of disparities.
Indeed, the most common techniques for estimating racial disparities using BISG probabilities are known to be biased when race is correlated with the outcome even after controlling on nanme and location [@chen2019fairness].
These approaches include the use of BISG probabilities as weights and thresholding the BISG probabilities to produce point estimates of individual race.
As formally discussed in Section \@ref(sec:id), these standard methods of racial disparity estimation based on BISG predictions also require individual's race to be conditionally independent of the outcome given their residence location, surnames, and other observable attributes.
...
Leveraging this identification strategy, we introduce a new class of models, Bayesian Instrumental Regression for Disparity Estimation (BIRDiE), that overcome the high-dimensionality of surnames to accurately estimate racial disparities.
...
For example, the most popular existing BISG disparity estimator pegs the gap at Democratic party registration between White and Black voters at `r round(-100*x$weight$disp_wb, 1)` percentage points, while the actual gap is `r round(-100*x$weight$disp_wb_true, 1)` percentage points---nearly double.
BIRDiE yields an estimate of `r round(-100*x$model$disp_wb, 1)` percentage points.
This represents about a `r round(100*bias_red)`% bias reduction.
...
Note that individual's race is unobservable but all other variables are assumed to be observed.
The availability of particular (or any) $X$ is not required for either the standard or proposed methodology.

We assume throughout that these variables are discrete, i.e., $|\cY|$, $|\cR|$, $|\cG|$, $|\cX|$, and $|\cS|$ are all finite.
...
\end{assump}
```
Like Assumption \ref{a:indep-sgx}, Assumption \ref{a:cens-acc} may never hold exactly in practice.
...
The plausibility of Assumption \ref{a:cens-acc} is stretched even further when the study population is a subset of the whole U.S. population, which is covered by the census data.
...
Even though Assumptions \ref{a:indep-sgx} and \ref{a:cens-acc} may not hold in practice, researchers find that BISG produces accurate and generally well-calibrated estimates in practice [@imai2016improving; @kenny2021das; @deluca2022validating].
We observe this as well in the validation study in Section \@ref(sec:valid).
...
Unfortunately, accurate and calibrated estimates of individual race predictions alone are not sufficient for unbiased estimation of racial disparities.
...
[^1]: An alternative is a *thresholding* or *classification* estimator, which deterministically assigns individuals to a predicted racial category based on the BISG estimates $\vb{\hat P}_i$ (either the largest $\hat P_{ir}$ or the one which exceeds a predetermined threshold).
    Unlike the weighting estimator, this classification estimator does not take into account prediction uncertainty, and its bias is harder to reason about than for the weighted estimator \citet{chen2019fairness}.
...
This causal structure implies the conditional independence relation $Y\indep R\mid G,X,S$ because all paths from $R$ to $Y$ are blocked by $G$, $X$, or $S$.
...
Specifically, we show that racial disparity is identifiable if one's surname is conditionally independent of the outcome given their race, residence geolocation, and other observed information, assuming that BISG can nonparametrically identify race probabilities.
We then develop a statistical model, BIRDiE, that estimates racial disparity under this identification condition while accounting for the high-dimensionality of surnames.
Finally, we propose a sensitivity analysis that addresses the potential violation of several of the key assumptions.
...
This causal structure is plausible in many real-world settings.
...
For example, if campaigns use the surnames of individual voters to decide whether to mobilize them, Assumption \ref{a:indep-ys} will be violated.
...
The country of origin for an immigrant may represent such a confounder.
...
```{=tex}
\begin{theoremrep}[Nonparametric Identification] \label{thm:id}
...
Specifically, we have $|\cY|\times|\cG|\times|\cX|\times|\cS|$ equations with $(|\cY|-1)\times|\cG|\times|\cX|\times|\cR|$ unknowns.
Since $|\cR| << |\cS|$, we can identify these unknowns as long as the linear system has sufficient rank. **CM FIX SYMBOL**
...
```{=tex}
\begin{theoremrep}[Unbiasedness of OLS Estimator] \label{thm:ols-bias}
...
Now, we can expand $\Pr(Y=y\mid R=r)$ as \[
    \Pr(Y=y\mid R=r) = \sum_{x\in\cX,g\in\cG} \Pr(Y=y\mid R=r, G=g, X=x)\Pr(G=g, X=x\mid R=r)
...
Now, since the individual BISG probabilities are nonnegative and sum to 1, a pair $j,k\in\cR$ of races has perfectly discriminating BISG probabilities if and only if the corresponding columns of $\hat{\vb P}$ are orthogonal, i.e., $\hat{\vb P}_{\cI(xg) j}^\top\hat{\vb P}_{\cI(xg) k}$.
...
In any case, if the analyst is interested in subgroup or small-area estimates of $\Pr(Y\mid R)$, the complete-pooling model will be of no help.
...
As with the complete-pooling model, the posterior of $\vb*\theta_{rgx}$ is conjugate to its prior, and so computation can be made very efficient.
Additionally, this model allows for any arbitrary relationship between $Y$, $R$, $G$, and $X$.
Since it is fully nonparametric, the posterior will converge to the true $\Pr(Y\mid R, G, X)$ with enough data.
However, in practice, the model suffers from the curse of dimensionality: the number of $(G, X)$ cells may be relatively large compared to the amount of available data, or even exceed it, especially since in practice $G$ will be quite large, covering many blocks or ZIP codes.
...
Often researchers only have available $S$ and $G$ for BISG inferences, with no additional covariates $X$.
In this common use case, we propose a hierarchical model, which maintains the flexibility of the saturated model while avoiding its high bias and variance in finite samples.
The hierarchical structure partially pools estimates of $\Pr(Y\mid R, G)$ towards an overall
estimate of $\Pr(Y\mid R)$, which in turn is pooled towards an overall estimate of $\Pr(Y)$.
This partial pooling allows the model to share information between geographic areas and racial categories, and should prove especially useful in cases where some areas or racial categories have few observations.

The proposed model is as follows. 
This particular hierarchical structure is inspired by @azzimonti2017hierarchical, though other parametrizations would accomplish the same goal. 
\begin{align*}
    Y_i\mid R_i, G_i, X_i, \Theta &\sim \Categorical_{\cY}(\vb*\theta_{R_iG_i}) \\
    \vb*\theta_{rg}\mid \phi_{\beta r},\vb*\beta_r  &\sim \Dirichlet(\phi_{\beta r}\vb*\beta_r) \\
    \vb*\beta_r\mid \phi_\alpha, \vb*\alpha &\iid \Dirichlet(\phi_\alpha\vb*\alpha) \\
    \vb*\alpha &\sim \Dirichlet(\vb*1) \\
    \phi_{\beta r} &\iid \Gam(2, \frac{2\sigma_\beta^2}{\tfrac{1}{4}-\sigma_\beta^2}), \quad
    \phi_\alpha \sim \Gam(2, \frac{2\sigma_\alpha^2}{\tfrac{1}{4}-\sigma_\alpha^2})
\end{align*}
Here $\phi_{\beta r}$ and $\phi_\alpha$ are effective sample sizes that control the amount of dispersion of the random effects $\vb*\theta_{rg}$ and $\vb*\beta_r$.
The priors on these parameters is written in terms of hyperparameters $\sigma_\beta$ and $\sigma_\alpha$, which represent the *a priori* expectation of the standard deviation of entries of $\vb*\theta_{rg}$ around $\vb*\beta_r$, and $\vb*\beta_r$ around $\vb*\alpha$.
For example, if we expect there to be little variation in $\vb*\theta_{rg}$ (i.e., the distribution of $Y\mid R$ is similar across levels of $G$) and moderate variation in $\vb*\beta_r$ (i.e., the distribution of $Y$ varies moderately across levels of $R$), we might set $\sigma_\beta=0.05$ and $\sigma_\alpha=0.4$.
This would give the $\phi_{\beta r}$ a prior mean of 99 and give $\phi_\alpha$ a prior mean of 24.
Both $\sigma_\beta$ and $\sigma_\alpha$ must lie in $(0, \half)$, since the maximum standard deviation of any entry in a Dirichlet-distributed vector is $\half$.

##### General mixed-effects model.

Finally, in cases with covariates $X$, a multinomial mixed-effects model can be used:
\begin{align*}
...
We also recommend including group-level covariates as fixed effects, which will help share information across random effects and significantly improve generalization performance to unseen random effect levels.
For example, if $G$ records counties, analysts could include fixed effects for racial and socioeconomic variables measured at the county level.
Ultimately the structure of this general model will have to be chosen based on the structure of the data and the relevant research question.
...
Since the individual race $\vb R$ is high-dimensional, discrete, and not of primary interest, we can marginalize it out as follows: \begin{align*}
    ...
    = \pi(\Theta) \prod_{i=1}^N \vb*\theta_{\cdot G_iX_iY_i}^\top \hat{\vb P}_i.
    \numberthis \label{eq:post-marg}
\end{align*} This makes it much easier to sample from the BIRDiE model, as the number of parameters is now a function only of the sizes of $\cY$, $\cR$, $\cG$, and $\cX$, and not the sample size $N$. 

The marginalized posterior in Equation \eqref{eq:post-marg} is simple, but not conjugate to any prior distribution.
It has only continuous parameters, but these are constrained to be nonnegative and to sum to 1 across certain dimensions.
The BIRDiE model can be fit with any general Bayesian inference procedure such as Markov chain Monte Carlo (MCMC).
...
In our applications here, <!--and in our open-source software implementation,--> we use stochastic variational inference (SVI) with a mean-field approximation to fit the BIRDiE model instead [@hoffman2013stochastic], as implemented in the Pyro probabilistic programming language [@bingham2018pyro].
...
BIRDiE, like the weighting and OLS estimators, relies upon Assumptions \ref{a:indep-sgx} and \ref{a:cens-acc}, which are required for the BISG race probabilities to be accurate.
Like the OLS estimators, it also crucially relies on \ref{a:indep-ys} for identification.
...
It is not possible to develop an analogous sensitivity analysis for Assumptions \ref{a:indep-ys}, since without it the model is not even identified.
...
We recommend that practitioners use other data sources, such as surveys, to estimate the overall race distribution, and use this instead of the national census estimates for $\vb q_R$.
...
We compare the performance of BIRDiE against those of other estimators.
...
The outcome we examine here, party registration, is the product of many unobservable factors, and are known to differ across racial groups.
...
#| fig.cap: |
#|     Distribution of party identification  by race, 
#|     for a sample of 500,000 North Carolina voters. Parties are Libertarian (LIB),
#|     Republican (REP), Independent (IND), and Democrat (DEM). 
...
To reduce computational burden, we further subsampled this file by selecting 200,000 records at random without replacement.
This sample size is large enough to ensure that sampling error is essentially negligible.
...
For a given set of BISG probabilities, we estimate the joint distribution of each outcome variable and race using the proposed method, the weighting estimator, the post-stratified OLS estimator, and a thresholding estimator that deterministically assigns each individual the maximum *a posteriori* racial category.
...
#|     party identification, by estimation method.
#|     All methods used ZIP code-level data for this figure; the results 
#|     for other levels of geographic detail are very similar.
...
For example, the true difference in Democratic identification between Black and White voters in the sample is $`r percent(-x$model$disp_wb_true, 0.1)`$ percentage points, meaning Black voters register Democratic at a much higher rate.
...
In contrast, the proposed model produces an estimate of $`r percent(-x$model$disp_wb, 0.1)`$ percentage points, which are only slightly lower than the ground truth.

Figure \@ref(fig:nc-disp) compares all the methods across all of these possible disparity measurements, using the ZIP code-level BISG predictions.
...
The OLS estimator (yellow triangles), which is based on our new identification strategy but, unlike the proposed model, does not account for certain probability constraints, gives erroneous results---especially for Hispanics, which represent a small minority in North Carolina.

```{r nc-tv, out.width="4.25in"}
#| fig.cap: |
...
Figure \@ref(fig:nc-tv) shows the TV distance for each estimator, not just for the ZIP code-level BISG estimates used in Figure \@ref(fig:nc-disp) but also across the range of geographic levels used in the BISG calculation (x-axis).
We find that BIRDiE (blue circles) outperforms every alternative<!--, for both party ID and turnout,--> at every geographic level.
...
When compared to the error of BIRDiE, their errors range from 169% to 368% greater<!--# for party registration and from XX% to XX% more for turnout-->, depending on the geography used for BISG predictions.
The OLS estimator (yellow triangles) generally outperforms the weighting and threshold estimators for party ID, but <!--# performs significantly worse for turnout than the weighting estimator--> still performs significantly worse than BIRDiE.

We do not find that finer geographic data necessarily translates to improved accuracy for the BIRDiE or conventional estimates.
While possibly counterintuitive, this finding underscores the fact that calibrated BISG probabilities, rather than highly precise probabilities, are all that is needed for accurate disparity estimation.
...
$$ Figure \@ref(fig:nc-tv-detail) in the appendix also shows these within-race TV distances, to illuminate how the estimators perform on each subgroup.
In general, BIRDiE is more accurate than the other methods for every racial group except "Other," though no method performs particularly well for that group.
In this example the BIRDiE model ran into some numerical issues and tended to generate estimates for the "Other" group on the boundary (i.e., assigning all of the "Other" group to a single party).
This likely explains the worsened accuracy for BIRDiE among this group, which makes up around 1.7% of the sample.
...
As noted above, the sampling error in all of these estimates is minimal---the widest 90% confidence interval in the ZIP code-level model estimates of the joint distribution of party ID by race is $(`r percent(x$ci_min, 0.01, suffix="\\%")`,`r percent(x$ci_max, 0.01, suffix="\\%")`)$---not enough to explain the residual TV error.
...
In particular, we can examine how large the average error in the individual BISG probabilities, $||\vb*\delta_i||$, would have to be in order to eliminate confidence in the estimated party ID disparity between voters.

```{r nc-bounds}
#| fig.cap: |
#|     Worst-case sensitivity analysis bounds, indicated by shaded areas,
#|     for estimated disparities between racial groups in party identification.
#|     ZIP code-level estimates are used.
#|     When the shaded region meets the x-axis, a worst-case error of the 
#|     indicated magnitude would be enough to eliminate the estimated disparity.
knitr::include_graphics("figures/nc_bounds.pdf")
```

Figure \@ref(fig:nc-bounds) shows how worst-case error in the individual BISG probabilities could change the estimated White--Black and White--Hispanic disparities in party identification.
Where the shaded regions meets the x-axis indicate the minimum error needed to eliminate the estimated disparity.
For example, a worst-case total error $\Delta$ corresponding to an average individual error $||\vb*\delta_i||$ of `r round(100*x$max_err_party_wb["dem"], 1)` percentage points would be enough to wipe out the estimated disparity in Democratic registration between White and Black voters.
In other words, the observed disparity of over 50 percentage points is robust to at least an error of this magnitude in the BISG probabilities.

Since the bounds are for worst-case error, the actual robustness to realistic modes and magnitudes of BISG error is likely significantly higher.
But these bounds provide a useful guarantee, and can can give an idea of the relative sensitivity of the estimates to data error.
For example, even though the estimated gap between White and Black voters in Republican registration is smaller than the gap for Democratic registration, both disparities are roughly equally robust to error.
In contrast, the disparity in independent registration could be eliminated by worst-case error on the order of `r round(100*x$max_err_party_wb["ind"], 1)` percentage points.
...
We have clarified the assumptions implicit in current approaches to disparity estimation from proxy data, and have introduced a new identifying assumption and accompanying model, BIRDiE.
...
Beyond the BISG probabilities, further empirical analyses could determine useful additional variables to condition on which could allow analysts to weaken Assumption \ref{a:indep-ys}.
The BIRDiE model too could be extended---to include additional covariates, or to directly model a continuous outcome variable.
Finally, there likely remain significant opportunities to improve computation for the existing BIRDiE model, so it can be fit more efficiently and robustly.
...
This section continues with the notational conventions of Section \@ref(sec:sens).
...
\end{align*} where $\vb Y=(\vb Y,\vb G, \vb X,\vb S)$ is the observed data.
...
In fact, since $w$ is a product of nonnegative terms, and the product of nonnegative monotonic functions is again monotonic, by the covariance inequality it suffices to show that each term \[
    w_i \dfeq 1 + \frac{\vb*\theta_{\cdot G_iX_iY_i}^\top \vb*\delta_i}{
        \vb*\theta_{\cdot G_iX_iY_i}^\top \hat{\vb P}_i} 
...
Partial derivatives with respect to elements of $\vec(\Theta)$ are clearly zero except for those corresponding to $\vec(\Theta)_{Y_i\cdot G_iX_i}$. These are \[
    ...
    = \frac{\vb*\theta_{\cdot G_iX_iY_i}^\top \hat{\vb P}_i\delta_{ir} -
        \vb*\theta_{\cdot G_iX_iY_i}^\top \vb*\delta_i \hat r_{ir}}{
        \vb*\theta_{\cdot G_iX_iY_i}^\top \hat{\vb P}_i}.
\] The directional derivative along $\vb d$ is then 
\begin{align*}
    \partial_{\vb d}w_i 
    &= \frac{\vb*\theta_{\cdot G_iX_iY_i}^\top \hat{\vb P}_i
                \vb*\delta_i^\top\vb d_{Y_i\cdot G_iX_i} - 
                \vb*\theta_{\cdot G_iX_iY_i}^\top \vb*\delta_i
                \hat{\vb P}_i^\top\vb d_{Y_i\cdot G_iX_i}}{
            (\vb*\theta_{\cdot G_iX_iY_i}^\top \hat{\vb P}_i)^2} \\
    &= \frac{\vb*\theta_{\cdot G_iX_iY_i}^\top}{
            (\vb*\theta_{\cdot G_iX_iY_i}^\top \hat{\vb P}_i)^2}
            (\hat{\vb P}_i\vb*\delta_i^\top\vb d_{Y_i\cdot G_iX_i} - 
            \vb*\delta_i\hat{\vb P}_i^\top\vb d_{Y_i\cdot G_iX_i}).
\end{align*}
Since $\vb*\theta_{\cdot G_iX_iY_i}$ is nonnegative, a sufficient condition for the directional derivative to be nonnegative, and thus for $w_i$ to be monotonically increasing in $\vec(\Theta)^\top\vb d$, is for
