...
The BISG(Z) procedure assumes that $S\indep G, Z\mid R$. With this assumption, \[
    \Pr(R=r\mid G=g, Z=z, S=s) \ \propto\ 
    \Pr(G=g, Z=z\mid R=r)\Pr(S=s \mid R=r)\Pr(R=r),
\] by Bayes' Rule.
The first and last terms are taken from decennial Census tables or the American Community Survey (ACS), while
the second term comes from the Census Bureau's surname tables [@censusnames].
...
Of course, other causal structures, such as a version of Figure \ref{fig:dag} with additional unobserved confounding between $G$, $Z$, $R$, and $X$, are possible; as long as $X\indep S\mid R,G,Z$, $\Pr(X=x\mid R=r)$ is identified, as we will see below.
...
Continue with the above notation but suppose that $X$ is binary. Then the usual weighted estimator of $\Pr[X=x\mid R=r]$ may be written \[
