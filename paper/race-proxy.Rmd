...
To reduce the potential bias of the weighted estimator, we propose an alternative identification assumption that may be applicable when Assumption \ref{a:indep-yr} is not credible.
...
```{=tex}
\begin{theoremrep}[Least Squares Estimator] \label{thm:ols-bias}
...
The next theorem shows that these two estimators are guaranteed to disagree, unless either the BISG probabilities perfectly discriminate or the weighted estimator is constant across races.
...
```{=tex}
\begin{theoremrep}[Necessary and Sufficient Condition for Equality of the Weighted and OLS Estimators] \label{thm:wtd-vs-ols}
...
**KI: I'm thinking that this sensitivity analysis is too unrealistic as you note below. I think we can cut it out (or put it in the appendix). I like the first sensitivity analysis and I think that's sufficient for this paper. **
...
**KI: I think we need a better explanation to help readers understand it.  How about mentioning one or two examples of $\vb d$? **
...
```{=tex}
\begin{theoremrep}
Let $r\in\cR$, $I\subseteq\{1,\dots,N\}$, and $\vb d$ a vector such that $\vb d_{Y_i\cdot G_iX_i}= c\vb e_r$ for all $i\in I$ and $0$ elsewhere, where $\vb e_r\in\R^{|\cR|}$ is the standard basis vector in the $r$ direction and $c>0$. {\bf KI: Should this $\vb d_{Y_i\cdot G_iX_i}$ be $\vb d_{y\cdot g x}$? If not, are you averaging over some population?  Explaining $\vb d$ before the theorem would make it easier to understand this.}
...
**KI: We should again help readers understand the next result better by giving an example of $\vb d$. **
...
To better understand how the proposed model performs in real-world contexts, we apply it to a North Carolina voter file which contains individual-level self-reported race and hence the "ground truth" relationship between outcome and race is known.
