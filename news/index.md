...
- Support for 2020 decennial census data
  ([\#19](https://github.com/CoryMcCartan/birdie/issues/19))
- Fix bug preventing `p_r="estimate"` in
  [`bisg_me()`](http://corymccartan.com/birdie/reference/bisg.md)
  ([\#18](https://github.com/CoryMcCartan/birdie/issues/18))
- Update CITATION
- Add `rstan` to `Suggests` due to its use in `Makefile`
  ([\#21](https://github.com/CoryMcCartan/birdie/issues/21))

## birdie 0.6.1

CRAN release: 2024-06-11

- Switch to an inverse-gamma prior for the random effects scale
  parameter in the mixed model.
- Add Jacobian adjustment to mixed model M step to improve accuracy and
  reduce prior sensitivity.
...
- [`simulate.bisg()`](http://corymccartan.com/birdie/reference/bisg.md)
  to perform multiple imputation from BISG probabilities
