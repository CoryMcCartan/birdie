...
Before we evaluate each method, it's worth checking that some kind of fancy
adjustment is in fact necessary. We can do so by seeing how much correlation
between race and party remains after conditioning on the naively-imputed race.
Unfortunately, there's quite a bit, indicating that the location-surname
imputations are missing some key sources of association between party and race.
Without further adjustment, we can expect estimates of $X\mid R$ to be off.
...
First, by visual inspection, the above model ("Stan") is closer to the true
joint distribution than the naive imputations.

```{r peek, results="hold"}
print_cond(xr$bisg, "bisg")
print_cond(xr$true, "true")
print_cond(xr$nonparam, "nonparam")
...
model explored here ("Stan"), since that model estimates $\Pr(X|R)$ directly,
and the $X$-margin is already extremely close to correct.
...
eval_joints(xr$true, "tv",
            ...
            #additive_raked = rake(xr$additive, p_x, p_r),
