...
\item Regression estimands
  \begin{itemize}
  \item short regression: $\Pr(Y_i = y \mid R_i = r)$
  \item long regression: $\Pr(Y_i = y \mid R_i = r, X_i =x)$
  \end{itemize}
...
= & - \frac{\E[\text{Cov}(\mathbf{1}\{Y_i = y\}, \mathbf{1}\{R_i = r\}\mid G_i,
          ...
          \end{align*}
          \begin{itemize}
          \item<2-> bias tends to be large for minority groups
          \item<3-> racial disparity tends to be underestimated
          \end{itemize}
          \vfill
        \item<4-> Required assumption: $$Y_i \indep R_i \mid G_i, S_i, X_i$$
      \end{itemize}
    \end{column}
    \onslide<4->{\begin{column}{0.425\textwidth}
  ...
  \end{column}}
  \end{columns}
\onslide<5->{Problem: race affects many aspects of the society}
