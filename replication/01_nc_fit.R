...
d = slice_sample(voters, n=500e3) %>%
    filter(reg_date <= as.Date("2012-11-01")) |>
...
if (!file.exists(path <- here("data-out/nc_fits_turnout.rds"))) {
    fits_turnout = imap(r_probs, function(d_pr, level) {
        ...
        model_race(d_pr, n_voted, !!rlang::sym(str_c("GEOID_", level)),
                   data=d, config=list(lr=lr, tol_rhat=1.15, draws=1000, it_avgs=500))
    })

    for (level in geo_levels[-1]) {
        d_pr = r_probs[[level]]
        cat(level, "\n")
        ...
        fits_turnout[[level]] = model_race(d_pr, n_voted, !!rlang::sym(str_c("GEOID_", level)),
                   data=d, config=list(lr=lr, tol_rhat=1.15, draws=1000, it_avgs=500,
                                       prior = list(x = 20.0, xr = 0.05, beta = 0.50)))
        gc()
    }
