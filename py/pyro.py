...
def model_additive(X, GZ, GZ_var, pr_base, N=1, n_r=5, n_x=2, n_gz=1, n_gz_var=1,
    ...
    with pyro.plate("N", N, subsample_size=subsamp) as ind:
        p_xr_i = normalizer(
            torch.einsum("ijk...,lmk...->lj...", GZ[None, ind], beta) + lp_xr.squeeze(-3)
            ) # (..., ind, n_r, n_x)
        #p_xr_i = normalizer(torch.tensordot(GZ[ind], beta[:, 0], ([-3], [-3])).squeeze() + lp_xr) # (ind, n_r, n_x)
        p_x_i = (pr_base[ind] @ p_xr_i).squeeze()
        if (zeros_row.shape[0] == 1): 
            x_dist = dist.Categorical(p_x_i, validate_args=False)
        else:
            x_dist = dist.Categorical(p_x_i[:, None, None, ...], validate_args=False)
            print(p_xr_i.shape, flush=True)
            print(pr_base.shape, flush=True)
            print(p_x_i.shape, flush=True)
            print(x_dist.batch_shape, flush=True)
            print(x_dist.event_shape, flush=True)
        pyro.sample("X", x_dist, obs=X[ind])
...
def fit_additive(X, GZ, GZ_var, pr_base, n_x=2, n_gz_var=1, 
        ...
        it=200, epoch=100, subsamp=1000, lr=0.01, tol_rhat=0.005):
    ...
    # rescale so ELBO ~= 1
    model = pyro.poutine.scale(model_additive, 1.0 / X.shape[0])
    guide = pyro.poutine.scale(guide, 1.0 / X.shape[0])
    ...
    for j in range(it):
        ...
        if j > 0 and j % epoch == 0:
            ...
            if converge_idx is None: # haven't convered yet
                r_hat_i = np.quantile(utils.R_hat_split(params, j, 0.5), 0.9)
                if r_hat_i <= tol_rhat:
                    converge_idx = j
        ...
        if converge_idx is not None: # have converged
            ...
            if j - converge_idx >= 10:
                break
    pbar.close()
    
    draws["params"] = {x: draws["params"][x].mean(0) for x in draws["params"]}
    pyro.get_param_store().set_state(draws)
    with pyro.plate("samples", 10, dim=-5):
        draws = guide(*m_args, **m_kwargs)
        #cond_model = pyro.condition(model, data=draws)
        #tr = pyro.poutine.trace(cond_model).get_trace(*m_args, **m_kwargs)
        #print(tr.compute_log_prob())
        #print(tr.log_prob)
        #sys.stdout.flush()
        #print(utils.model_log_prob(model_additive, draws, *m_args, **m_kwargs))
    draws = {x: draws[x].cpu().detach().numpy() for x in draws}
    lw, mt, gt = pyro.infer.importance.vectorized_importance_weights(model, guide, *m_args, **m_kwargs, num_samples=10, max_plate_nesting=3)
    print(lw, flush=True)
        
    # Average p_xr over GZ
    p_x = draws["p_x"].squeeze(-2)
    lp_xr_raw = draws["lp_xr_raw"]
    beta_scale = draws["beta_scale"]
    beta_raw = draws["beta_raw"]
    
    beta = beta_scale[..., GZ_var.detach().numpy(), :, :] * beta_raw 
    zero_row = np.zeros((p_x.shape[0], 1, n_r, 1))
    ...
    if False:
        wt = pyro.infer.importance.vectorized_importance_weights(
